...
import { deleteFromCart, deleteAllFromCart, clearCart } from '../../redux/cartSlice'; // Added clearCart
...
function Cart() {
  ...
  const decreaseQuantity = (id) => {
    ...
    // Show appropriate toast message based on remaining quantity
    if (groupedItems[id].quantity <= 1) {
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-gray-100 pt-5 pb-20" style={darkModeStyle}>
        <h1 className="mb-10 text-center text-2xl font-bold">Cart Items</h1>
        
        {Object.keys(groupedItems).length === 0 ? (
          <div className="text-center py-10">
            <h2 className="text-xl font-semibold">Your cart is empty</h2>
            <p className="mt-2 text-gray-500" style={{ color: mode === 'dark' ? '#A0AEC0' : '' }}>
          ...
          <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
            <div className="rounded-lg md:w-2/3">
              {Object.values(groupedItems).map((item) => {
                ...
                return (
                  <div key={id} className="justify-between mb-6 rounded-lg border drop-shadow-xl bg-white p-6 sm:flex sm:justify-start" style={darkModeCardStyle}>
                    <img src={imageUrl} alt="product-image" className="w-full rounded-lg sm:w-40 object-cover" />
                    <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                      <div className="mt-5 sm:mt-0">
                        <h2 className="text-lg font-bold text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>{title}</h2>
                        <h2 className="text-sm text-gray-700" style={{ color: mode === 'dark' ? '#A0AEC0' : '' }}>{description}</h2>
                        <p className="mt-1 text-md font-semibold text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{price}</p>
                      </div>
                      <div className="mt-4 flex flex-col justify-between items-end sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                        <div className="flex items-center border-gray-100">
                          ...
                          <span className="h-8 w-8 border flex items-center justify-center bg-white text-center text-xs outline-none" style={{ color: mode === 'dark' ? 'black' : '' }}>
                            {quantity}
                          </span>
                          <button 
                        ...
                        <div onClick={() => deleteCart(id)} className="cursor-pointer mt-4">
            ...
            <div className="mt-6 h-fit rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={darkModeCardStyle}>
